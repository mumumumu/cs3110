    ...
    match e_lst with 
      ...
      | h::t -> let n1,n2,w = h in
                ...
                let _ = 
                  	...
                  	D.union s a b;
                  	threshold.(a) <- w + t_c(D.size u a) in
                helper t
  in helper sort_edges
...
let mergeSmallComponents (u : segmentation) (edges : weighted_edge list)
  ...
  let rec helper e_lst =
    match e_lst with
      ...
      | h::t -> 
					let n1,n2,w = h in
          let a = D.find s n1 and let b = D.find s n2 in
          if ((a != b) && ((D.size s a < minSize) || (D.size s b < min_size))) then
          	D.union s a b;
          helper t
  in helper edges
...
let segmentImage (img : I.image) (sigma : float) (k : float)
  ...
  let diff x1 y1 x2 y2 = 
    let r1,g1,b1 = (getPixel simg x1 y1) 
		and r2,g2,b2 = (getPixel simg x2 y2) in
    sqrt (((r1-r2)**2) + ((g1-g2)**2) + ((b1-b2)**2)) in
...
let colorComponents (img : I.image) (u : segmentation) : I.image =
  ...
  let colors = Array.make (w*h) (0.0,0.0,0.0) in
	let _ = I.fold (fun x y c _ -> let i = D.find u (w*y+x) in
    														 let r1,g1,b1 = colors.(i) and r2,g2,b2 = c in
    														 colors.(i) <- (r1+r2,g1+g2,b1+b2))
		  _ img in
	let avg_colors = Array.mapi 
			(fun i a -> let r,g,b = a and n = float_of_int (D.size u i) in
    	(r/.n,g/.n,b/.n))	a in
	I.map (fun x y c -> avg_colors.(D.find u (w*y+x))) img
